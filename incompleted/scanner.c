...
void skipBlank() {
  // TODO
  while ((currentChar != EOF) && (charCodes[currentChar] == CHAR_SPACE || currentChar == '\n' || currentChar == '\t'))
    readChar();
...
Token* readIdentKeyword(void) {
  ...
  a[i] =0;
  TokenType type = checkKeyword(a);
  if (type == TK_NONE) {
    for (int j = 0; j <= i; j++) {
      token->string[j] = a[j];
    }
    return token;
  } else token->tokenType = type;
  return token;
...
Token* getToken(void) {
  ...
  case CHAR_COLON:
    ...
    if (charCodes[currentChar] == CHAR_EQ) {
      ...
      readChar();
      return token;
    } else return token = makeToken(SB_COLON, lineNo, colNo - 1);
